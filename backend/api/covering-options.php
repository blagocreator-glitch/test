...
$body = readJsonBody();
$catalogKey = trim((string)($body['catalog'] ?? 'floors.common'));
...
$selected = is_array($selectedLabels) ? array_map('strval', $selectedLabels) : [];
$selectedSet = [];
...
$coverKey = $catalogKey === 'ceilings.common'
    ? ceilingCoverDetectKey($cover)
    : ($catalogKey === 'walls.common' ? wallCoverDetectKey($cover) : floorCoverDetectKey($cover));
$formatKey = $catalogKey === 'ceilings.common'
    ? ceilingCoverSelectedFormatKey($selected)
    : ($catalogKey === 'walls.common' ? wallCoverSelectedFormatKey($selected) : floorCoverSelectedFormatKey($selected));
$requiredGroups = $catalogKey === 'ceilings.common'
    ? ceilingCoverRequiredGroups($catalog, $cover, $selected)
    : ($catalogKey === 'walls.common'
        ? wallCoverRequiredGroups($catalog, $cover, $selected)
        : floorCoverRequiredGroups($catalog, $cover, $selected));

$groups = [];
foreach (($catalog['optionGroups'] ?? []) as $group) {
    $options = [];
    foreach (($group['options'] ?? []) as $option) {
        $isAllowed = $catalogKey === 'ceilings.common'
            ? ceilingCoverOptionAllowed($option, $coverKey, $formatKey)
            : ($catalogKey === 'walls.common'
                ? wallCoverOptionAllowed($option, $coverKey, $formatKey)
                : floorCoverOptionAllowed($option, $coverKey, $formatKey));
    ...
    $options = array_values(array_filter($options, fn($item) => $item['label'] !== ''));
    if (!$options) continue;
    $groups[] = [
